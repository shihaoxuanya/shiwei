...
def source_card(row):
    ...
    return {
        ...
        "originalUrl": row["original_url"] if "original_url" in keys else None,
        "finalUrl": row["final_url"] if "final_url" in keys else None,
        "capturedAt": row["captured_at"] if "captured_at" in keys else None,
        "bodyHash": row["body_hash"] if "body_hash" in keys else None,
