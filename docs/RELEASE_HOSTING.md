...
2026-09-06 已进一步部署第一方基础统计及数据概览，保留原管理员与版本数据。桌面版本同步到 0.3.1，新生产安装默认开启基础统计、旧关闭状态保留；最新 383 项回归和公网验收见 [ACCEPTANCE-ANALYTICS.md](ACCEPTANCE-ANALYTICS.md)。以下为此前发布基础设施状态，不意味着正式签名发版已完成。
...
服务器 SSH 已登录，域名解析正确；后台已部署到 `https://zhishimanghe.com/admin`，真实 HTTPS 登录页与证书验证通过。管理员已初始化，真实登录、受保护接口及退出验证通过。正式 Secrets 和 A→B 升级仍未完成，详见 `ACCEPTANCE-CONTROL-PLANE.md`；不以本地测试页面替代线上验收。
